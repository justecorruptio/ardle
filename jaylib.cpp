...
void Jaylib::smallPrint(uint8_t x, uint8_t y, const uint8_t * str, uint8_t color) {
    ...
    for(;c = *str ++;) {
        c -= 65;
        drawBand(128 - y, x, HORIZ_SMALL_CHARS + 5 * c, 5, color);
        x += 6;
    }
}
...
void Jaylib::drawPixel(int16_t x, int16_t y, uint8_t color) {
    ...
    drawBand(127 - y - 1, x, HORIZ_LARGE_CHARS + 7 * ('L' - HORIZ_OFFSET) + 1, 1, color);
}

void Jaylib::drawLargeCharBand(int16_t x, int16_t y, uint8_t offset, uint8_t l, uint8_t color) {
    drawBand(127 - y - l, x, HORIZ_LARGE_CHARS + offset, l, color);
}
